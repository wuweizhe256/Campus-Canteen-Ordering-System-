...
| `moving_to_table` | `v0_current` | 当前代码中表示前往座位 |
...
| `seats` | `array<integer/null>` | `v0_current` | 当前座位数组；元素为学生 `id` 或 `null` |
...
| --- | --- | --- |
| `null` | `v0_current` | 空闲 |
| `student.id` | `v0_current` | 当前代码中表示占用；实际是在分配座位时提前写入 |
...
| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `tray_return_points` | `array<TrayReturnFrame>` | `P0_planned` | 碗筷收集点 |
| `stats` | `StatsFrameP0` | `P0_planned` | P0 基础统计 |

### 3.2 学生字段调整

| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `table_id` | `integer/null` | `P0_planned` | 当前目标餐桌 |
| `seat_index` | `integer/null` | `P0_planned` | 当前目标座位 |

P0 新增 / 替换学生状态：

| 值 | 阶段 | 说明 |
| --- | --- | --- |
| `searching_seat` | `P0_planned` | 找座中，负责选座 |
| `moving_to_seat` | `P0_planned` | 前往座位，替代当前 `moving_to_table` |
| `moving_to_tray_return` | `P0_planned` | 前往碗筷收集处 |

兼容说明：

- `moving_to_table` 是 `v0_current` 状态。
- P0 后建议前端支持 `moving_to_table` 和 `moving_to_seat`，直到后端完全替换。

### 3.3 餐桌 / 座位接口调整

P0 建议将 `tables[].seats` 从 `array<integer/null>` 升级为 `array<SeatFrame>`。

| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `index` | `integer` | `P0_planned` | 座位索引 |
| `status` | `string` | `P0_planned` | 座位状态 |
| `student_id` | `integer/null` | `P0_planned` | 占用或预留学生编号 |
...
| --- | --- | --- |
| `free` | `P0_planned` | 空闲 |
| `reserved` | `P0_planned` | 已分配但学生未到达 |
| `occupied` | `P0_planned` | 学生已到达并就座 |

### 3.4 碗筷收集点 `TrayReturnFrame`

| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `id` | `integer` | `P0_planned` | 收集点编号 |
| `x` | `number` | `P0_planned` | 中心横坐标 |
| `y` | `number` | `P0_planned` | 中心纵坐标 |
| `width` | `number` | `P0_planned` | 区域宽度 |
| `height` | `number` | `P0_planned` | 区域高度 |
| `is_congested` | `boolean` | `P0_planned` | 是否拥堵 |

### 3.5 P0 统计 `StatsFrameP0`

| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `avg_wait_time` | `number/null` | `P0_planned` | 平均等待时间：进入窗口队列到完成出餐 |
| `avg_total_time` | `number/null` | `P0_planned` | 平均就餐总耗时：学生入场到学生离场 |
| `max_active_students` | `integer` | `P0_planned` | 仿真过程中最大在场人数 |
| `stall_queue_stats` | `array<StallQueueStats>` | `P0_planned` | 各窗口队列统计 |
| `seat_utilization` | `number/null` | `P0_planned` | 总座位利用率 |

`StallQueueStats`：

| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `stall_id` | `integer` | `P0_planned` | 窗口编号 |
| `max_queue_length` | `integer` | `P0_planned` | 最大队列长度 |
...
| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `event_type` | `string` | `P0_planned` | 事件类型 |
| `game_time` | `number` | `P0_planned` | 事件发生的仿真时间 |
| `student_id` | `integer/null` | `P0_planned` | 相关学生编号 |
| `stall_id` | `integer/null` | `P0_planned` | 相关窗口编号 |
| `table_id` | `integer/null` | `P0_planned` | 相关餐桌编号 |
| `seat_index` | `integer/null` | `P0_planned` | 相关座位编号 |
| `from_state` | `string/null` | `P0_planned` | 事件发生前学生状态 |
| `to_state` | `string/null` | `P0_planned` | 事件发生后学生状态 |

P0 事件类型枚举：

| 值 | 阶段 | 说明 | 用于指标 |
| --- | --- | --- | --- |
| `student_spawned` | `P0_planned` | 学生入场 | 平均就餐总耗时、最大在场人数 |
| `queue_started` | `P0_planned` | 学生开始排队 | 平均等待时间、窗口队列统计 |
| `food_ready` | `P0_planned` | 学生完成出餐 | 平均等待时间 |
| `eating_started` | `P0_planned` | 学生开始就餐 | 座位利用率 |
| `eating_finished` | `P0_planned` | 学生结束就餐 | 座位利用率 |
| `student_left` | `P0_planned` | 学生离场 | 平均就餐总耗时、最大在场人数 |
...
| 字段 | 类型 | 阶段 | 来源事件 / 数据 |
| --- | --- | --- | --- |
| `avg_wait_time` | `number/null` | `P0_planned` | `queue_started`、`food_ready` |
| `avg_total_time` | `number/null` | `P0_planned` | `student_spawned`、`student_left` |
| `max_active_students` | `integer` | `P0_planned` | `student_spawned`、`student_left` |
| `stall_queue_stats[].max_queue_length` | `integer` | `P0_planned` | `queue_count` 采样或窗口队列事件 |
| `seat_utilization` | `number/null` | `P0_planned` | `eating_started`、`eating_finished` |

P0 不加入的统计字段：

- `lost_students`：业务文档中属于直接离场 / 不吃就走的扩展统计，暂不作为 P0 必需输出。
- `avg_queue_length`：业务文档中提到“平均队列长度当前没有累计”，但 P0 阶段只要求各窗口最大队列长度。
...
| 字段 | 类型 | 阶段 | 说明 |
| --- | --- | --- | --- |
| `entrance_flow` | `array<object>` | `P3_planned` | 各入口流量 |
| `exit_flow` | `array<object>` | `P3_planned` | 各出口流量 |
...
1. 第一版只实现 `v0_current` 字段，保证能跑当前程序。
2. 前端数据层保留适配层，把 `door` 适配成单个入口，把 `exit` 适配成单个出口。
3. 前端座位展示先支持 `array<integer/null>`，同时预留 `array<SeatFrame>` 分支。
4. 学生状态展示同时兼容 `moving_to_table` 和后续 `moving_to_seat`。
...
| `tables[].seats: array<integer/null>` | `tables[].seats: array<SeatFrame>` | `P0_planned` |
| `moving_to_table` | `searching_seat`, `moving_to_seat` | `P0_planned` |
